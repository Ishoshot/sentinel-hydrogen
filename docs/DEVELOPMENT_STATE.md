...
> **Purpose**: This document provides persistent memory across development sessions. It contains completed work only.
>
> **For upcoming work**, see `DEVELOPMENT_STATE_NEW.md`.
...
### BYOK Entities (Phase 6 - Implemented)

| Entity          | Description                                                    |
| --------------- | -------------------------------------------------------------- |
| **ProviderKey** | BYOK credential for AI provider (encrypted, repository-scoped) |
...
### Phase 6: BYOK Provider Key Management

**Status: COMPLETE**

Secure frontend-based API key management for Bring-Your-Own-Key (BYOK) AI providers. Replaces the original config-file approach (which would have exposed keys in version control) with encrypted database storage and role-based access control.

-   [x] **Security Architecture**
    -   [x] API keys encrypted at rest using Laravel's `encrypted` cast
    -   [x] Keys NEVER exposed in API responses (`$hidden` attribute)
    -   [x] `#[SensitiveParameter]` attribute prevents keys in stack traces
    -   [x] HTTPS-only transmission (no keys in URLs or logs)
    -   [x] Role-based access (only owners/admins can manage keys)
-   [x] **Backend API**
    -   [x] `ProviderKeyController` (index, store, destroy)
    -   [x] `StoreProviderKeyRequest` form request with validation
    -   [x] `ProviderKeyResource` (never includes encrypted_key)
    -   [x] `StoreProviderKey` action (upsert pattern with activity logging)
    -   [x] `DeleteProviderKey` action (with activity logging)
    -   [x] `ProviderKeyPolicy` (viewAny, view, create, delete)
    -   [x] Added `providerKeys()` HasMany relationship to Repository model
-   [x] **Activity Logging**
    -   [x] Added `ProviderKeyUpdated` and `ProviderKeyDeleted` to ActivityType enum
    -   [x] Activity logs include provider name only (never key values)
-   [x] **API Routes**
    -   [x] `GET /api/workspaces/{workspace}/repositories/{repository}/provider-keys`
    -   [x] `POST /api/workspaces/{workspace}/repositories/{repository}/provider-keys`
    -   [x] `DELETE /api/workspaces/{workspace}/repositories/{repository}/provider-keys/{providerKey}`
-   [x] **Tests**
    -   [x] 19 comprehensive tests in `ProviderKeyManagementTest.php`
    -   [x] Tests verify: role-based access, upsert behavior, activity logging, no key exposure
-   [x] **Documentation**
    -   [x] Created `FRONTEND_HANDOVER_BYOK_PROVIDER_KEYS.md` for frontend team
    -   [x] Updated `.sentinel/config.example.yaml` (removed providers section)
    -   [x] Updated `docs/SENTINEL_CONFIG.md` (added dashboard-based key management note)
-   [x] **Config Sync**
    -   [x] Synced `config/reviews.php` with `SentinelConfig::default()` values
    -   [x] Removed legacy `summary_only` rule (not used by system prompt)
    -   [x] Changed defaults: enabled_rules, severity thresholds, max_findings, tone, language
    -   [x] Fixed `ReviewPolicyResolver` to replace (not merge) enabled_rules
    -   [x] Updated 12 tests in `ReviewPolicyResolverTest.php`

**Design Decision**: Keys are stored per-repository (not workspace-wide) to allow different repos to use different AI providers or keys.

### Phase 7: Plans & Billing

**Status: NOT STARTED**

See `DEVELOPMENT_STATE_NEW.md` for detailed implementation plan.

-   [ ] Plan model with subscription tiers
...
### BYOK Provider Key Management (Phase 6 - Complete)

#### Database Tables Created

```
provider_keys (id, workspace_id, repository_id, provider, encrypted_key, created_at, updated_at)
```

**Security Features:**

-   `encrypted_key` column uses Laravel's `encrypted` cast (AES-256-CBC encryption)
-   `encrypted_key` in model's `$hidden` array (never serialized to JSON)
-   Workspace-scoped for multi-tenancy
-   Repository-scoped (different repos can use different keys/providers)

#### Models

-   `App\Models\ProviderKey` - workspace, repository, provider enum, encrypted key storage

#### Actions

-   `App\Actions\ProviderKeys\StoreProviderKey` - Upsert pattern (one key per provider per repo) with activity logging
-   `App\Actions\ProviderKeys\DeleteProviderKey` - Delete with activity logging

#### Controllers

-   `App\Http\Controllers\ProviderKeyController` - index (list keys), store (add/update key), destroy (delete key)

#### API Resources

-   `App\Http\Resources\ProviderKeyResource` - NEVER includes encrypted_key (only id, provider, timestamps)

#### Form Requests

-   `App\Http\Requests\ProviderKey\StoreProviderKeyRequest` - Authorization via `canManageSettings()`, validates provider and key

#### Routes (routes/api.php - BYOK)

```php
// Provider Keys (BYOK) - workspace and repository scoped
GET    /api/workspaces/{workspace}/repositories/{repository}/provider-keys
POST   /api/workspaces/{workspace}/repositories/{repository}/provider-keys
DELETE /api/workspaces/{workspace}/repositories/{repository}/provider-keys/{providerKey}
```

#### Policies

-   `ProviderKeyPolicy` - viewAny, view, create (owners/admins only), delete (owners/admins only)

#### Activity Types (BYOK Events)

-   `ProviderKeyUpdated` - API key configured/updated for repository (provider name logged, never key value)
-   `ProviderKeyDeleted` - API key removed from repository

#### Tests (Phase 6)

-   `tests/Feature/ProviderKeys/ProviderKeyManagementTest.php` (19 tests)
    -   Role-based access control (owners/admins can manage, members can view)
    -   Upsert behavior (same provider updates existing key)
    -   Activity logging for create/delete
    -   Cross-workspace isolation (404 on invalid workspace)
    -   API response NEVER includes encrypted_key
    -   Policy enforcement on all endpoints

#### Security Guarantees

1. **Keys never in responses** - Model's `$hidden` attribute prevents serialization
2. **Keys never logged** - Only provider names in activity logs, never key values
3. **Encrypted at rest** - Laravel's `encrypted` cast (AES-256-CBC)
4. **HTTPS only** - Keys transmitted over secure connection
5. **Role-based access** - Only owners/admins can manage keys (canManageSettings permission)
6. **Workspace scoping** - Keys isolated per workspace
7. **No key retrieval** - Users can only see that a key exists, not its value
8. **Sensitive parameter** - `#[SensitiveParameter]` prevents key in stack traces (PHP 8.2+)

#### Integration with Review System

-   `App\Services\Reviews\ProviderKeyResolverService` - Resolves BYOK keys for repositories
-   `App\Exceptions\NoProviderKeyException` - Thrown when no key configured (gracefully skips review)
-   `App\Actions\Reviews\ExecuteReviewRun` - Catches `NoProviderKeyException` and marks run as `Skipped` (not `Failed`)
-   Skipped runs include `skip_reason: 'no_provider_keys'` in metadata

#### Config Sync with SentinelConfig

-   Synced `config/reviews.php` with `SentinelConfig::default()` / `ReviewConfig::default()` values
-   Removed legacy `summary_only` from `enabled_rules` (not used by system prompt)
-   Updated defaults:
    -   `enabled_rules`: `['security', 'correctness', 'performance', 'maintainability']` (was `['summary_only']`)
    -   `severity_thresholds.comment`: `'low'` (was `'medium'`)
    -   `comment_limits.max_inline_comments`: `25` (was `10`)
    -   Added: `tone`, `language`, `focus` fields
-   Fixed `ReviewPolicyResolver::mergeReviewConfig()` to **replace** (not merge) enabled_rules
    -   Allows users to disable default categories (e.g., `performance: false` now works correctly)
-   Updated 12 tests in `ReviewPolicyResolverTest.php` to match new behavior
...
-   `handover/FRONTEND_HANDOVER_BYOK_PROVIDER_KEYS.md` - Phase 6 (BYOK Provider Key Management)
...
_Phases Completed: Identity & Workspace Foundation (Phase 1), Source Control Integration (Phase 2), Review System Core (Phase 4), Context Engine (Phase 4.5), Repository Configuration (Phase 5), BYOK Provider Key Management (Phase 6)_
_Infrastructure: Queue System (Redis + Horizon), GitHub API Rate Limiting_
_Tests: 499 passing (including 19 provider key tests)_
_Next Phase: See DEVELOPMENT_STATE_NEW.md for upcoming work_
