 ...
 */
final readonly class CodeEmbeddingSymbolChunkExtractor
{
    /**
    ...
    public function __construct(
        ...
        private CodeEmbeddingLineRangeExtractor $lineRangeExtractor = new CodeEmbeddingLineRangeExtractor,
    ) {}
    ...
    public function extract(CodeIndex $codeIndex, array $structure): array
    {
        ...
        if (is_array($classes)) {
            foreach ($classes as $class) {
                ...
                $classContent = $this->lineRangeExtractor->extract($codeIndex->content, $startLine, $endLine);

                if ($className !== null && $classContent !== '') {
                    $chunks[] = [
                        'type' => ChunkType::ClassChunk,
                        'symbol_name' => $className,
                        'content' => $this->contentFormatter->formatSymbolChunk('class', $className, $classContent, $codeIndex->file_path),
                        'metadata' => [
                ...
                if (is_array($methods)) {
                    foreach ($methods as $method) {
                        ...
                        $methodContent = $this->lineRangeExtractor->extract($codeIndex->content, $methodStartLine, $methodEndLine);
                        ...
                        if ($className !== null && $methodName !== null && $methodContent !== '') {
                            ...
                            $chunks[] = [
                                ...
                                'content' => $this->contentFormatter->formatSymbolChunk('method', $fullMethodName, $methodContent, $codeIndex->file_path),
                                'metadata' => [
                                    'file_path' => $codeIndex->file_path,
                                    'class' => $className,
                                    'method' => $methodName,
        ...
        if (is_array($functions)) {
            foreach ($functions as $function) {
                ...
                $functionContent = $this->lineRangeExtractor->extract($codeIndex->content, $funcStartLine, $funcEndLine);

                if ($functionName !== null && $functionContent !== '') {
                    $chunks[] = [
                        'type' => ChunkType::Function,
                        'symbol_name' => $functionName,
                        'content' => $this->contentFormatter->formatSymbolChunk('function', $functionName, $functionContent, $codeIndex->file_path),
                        'metadata' => [
                            'file_path' => $codeIndex->file_path,
